...
#                 help = 'path to yolo pre-trained weights', default='custom/yolov3-tiny_final.weights')
# ap.add_argument('-cl', '--classes', 
#                 help = 'path to text file containing class names',default='custom/objects.names')
# args = ap.parse_args()
# ap.add_argument('-c', '--config', 
#                help = 'path to yolo config file', default='custom/lp-recognition_new.cfg')
# ap.add_argument('-w', '--weights', 
#                help = 'path to yolo pre-trained weights', default='custom/lp-recognition_350000.weights')
# ap.add_argument('-cl', '--classes', 
#                help = 'path to text file containing class names',default='custom/lp-recognition.names')
# args = ap.parse_args()

# args = {
#     "weights": 'custom/lp-recognition_350000.weights',
#     "classes": 'custom/lp-recognition.names',
#     "config": 'custom/lp-recognition_new.cfg'

# }

class Args:
    def __init__(self):
        self.weights = 'custom/lp-recognition.backup'
        self.classes = 'custom/lp-recognition.names'
        self.config = 'custom/lp-recognition.cfg'
...
def draw_pred(img, class_id, confidence, x, y, x_plus_w, y_plus_h):
    ...
    cv2.putText(img, label, (x-1,y-1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
...
test_file = "custom/test.txt" 
if __name__ == "__main__":
    for file in open(test_file).readlines():
        file = file[:-1]
        ...
        cv2.putText(image, label, (0, 5), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 0, 0))
        plt.imshow(image)
        plt.show()
        # cv2.imshow(window_title, image)

def character_detection(plate_image):
    image = plate_image
    if image.shape != 3:
        return 
    blob = cv2.dnn.blobFromImage(image, 1.0/255.0, (416,416), [0,0,0], True, crop=False)
    ...
    for i in indices:
        ...
        h = box[3]
        # print()
        results.append((x, y, w, h, str(classes[class_ids[i]])))
        # draw_pred(image, class_ids[i], confidences[i], round(x), round(y), round(x+w), round(y+h))
